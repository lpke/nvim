...
return function(tool, tools)
  ...
  -- Strip a leading `cd <path> && ` when the target directory is
  -- at or below the current working directory.  This lets the
  -- actual command that follows be matched against the pattern
  -- lists below.
  local cwd = vim.fn.getcwd()
  cmd = cmd:gsub('^cd%s+(.-)%s*&&%s*', function(dir)
    -- Resolve the target; treat relative paths from cwd.
    local target = vim.fn.fnamemodify(dir, ':p'):gsub('/+$', '')
    local norm_cwd = cwd:gsub('/+$', '')
    if target == norm_cwd or target:sub(1, #norm_cwd + 1) == norm_cwd .. '/' then
      return '' -- safe cd; strip it
    end
    return 'cd ' .. dir .. ' && ' -- outside cwd; keep as-is
  end)
